...
| 公开仓库 | 用户已确认公开创建 `AceYKN/nailoong-naiwa`；当前 HEAD `cfc83c2286204fdec47c01e522458e547cff67cb` 对应 CI run `34888421124` 六项全部通过，隔离暂存副本与 `origin/main` 已同步。最近一次代码验证提交为 `c1b04a376051bb7ce78c42eb80e209ad2f352c48`。 |
| Windows 验证 artifact | 最新 CI run `34888421124` 的 `rust-windows-opencv` 上传步骤成功；artifact 名为 `nlnf-windows-opencv-cfc83c2286204fdec47c01e522458e547cff67cb`，大小 46,879,078 bytes，当前未过期，保留至 2026-09-28。 |
...
| 范围 | 状态 |
| --- | --- |
| OpenCV 原生环境 | 用户态工具目录和公开 CI 均已用固定 OpenCV 4.13.0 + LLVM 20.1.8 验证；最近一次代码验证提交 `c1b04a376051bb7ce78c42eb80e209ad2f352c48` 的 CI run `34886669194` 已通过，文档同步后的当前 HEAD `cfc83c2286204fdec47c01e522458e547cff67cb` 的 run `34888421124` 也已通过；开发环境和 OpenCV 专用 NSIS staging 已可复现，第二台机器、Defender 和签名仍需独立验证。 |
| `OTHER` / `UNKNOWN` | 已区分：低分且两类均无几何 inlier 返回 `OTHER`；模糊或有部分证据但不足以确定时返回 `UNKNOWN`。仍需真实负例冻结集校准边界。 |
| 公开 GitHub | `AceYKN/nailoong-naiwa` 已确认公开；当前 HEAD `cfc83c2286204fdec47c01e522458e547cff67cb` 的 GitHub CI run `34888421124` 中，仓库卫生、前端、Rust Ubuntu、Rust Windows、Windows Tauri 打包和 Windows OpenCV 原生 feature job 全部成功。 |
