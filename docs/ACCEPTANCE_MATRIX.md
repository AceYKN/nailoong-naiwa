...
| Rust 质量门 | 默认 feature 的 fmt/clippy 通过；公开仓库当前提交 `9110402ea511aa05a6351bb0afcc52f0b28e1605` 的 Windows OpenCV feature CI run `34872691473` 通过 84 项库测试、clippy、Tauri OpenCV NSIS 构建、runtime staging、资源检查和 artifact 上传；release feature 的结构合法测试凭证绑定规则仍保留，缺少凭证或证书 SHA 与当前 checkout 不一致时会拒绝编译。证书现同时绑定完整 Reference Bank、manifest、视觉/缓存指纹和 OpenCV runtime SHA-256。 |
...
| 公开仓库 | 用户已确认公开创建 `AceYKN/nailoong-naiwa`；当前提交 `9110402ea511aa05a6351bb0afcc52f0b28e1605` 对应 CI run `34872691473` 六项全部通过，隔离暂存副本与 `origin/main` 已同步。 |
| Windows 验证 artifact | CI run `34872691473` 的 `rust-windows-opencv` 上传步骤成功；artifact 名为 `nlnf-windows-opencv-9110402ea511aa05a6351bb0afcc52f0b28e1605`，大小 46,854,783 bytes，当前未过期，保留至 2026-09-28。 |

## 已实现但尚未正式验收

| 范围 | 当前状态与缺失证据 |
| --- | --- |
| OpenCV SIFT | `vision_opencv.rs` 已实现 pHash 一级短路、SIFT 主路径、SIFT 无可用描述子时的 AKAZE fallback、按描述子类型选择 BFMatcher 距离、Lowe ratio、RANSAC Homography、coverage、reprojection error、调试匹配图、版本化颜色/描述子缓存和明显色差负向辅助惩罚；描述子缓存现绑定实际源图 SHA-256、提取器种类与提取器指纹，预测缓存绑定源码/配置/采样引擎指纹。公开 CI 以固定 OpenCV 4.13.0 + LLVM 20.1.8 完成 Windows 原生 `check/test/clippy`，84 项 feature 库测试通过，并通过 Tauri OpenCV NSIS 构建、runtime staging 和安装包资源检查。pHash 短路没有几何证据，因此不满足 QQ 撤回门槛。 |
...
| 范围 | 状态 |
| --- | --- |
| OpenCV 原生环境 | 用户态工具目录和公开 CI 均已用固定 OpenCV 4.13.0 + LLVM 20.1.8 验证；公开仓库当前提交 `9110402ea511aa05a6351bb0afcc52f0b28e1605` 的 CI run `34872691473` 已通过；开发环境和 OpenCV 专用 NSIS staging 已可复现，第二台机器、Defender 和签名仍需独立验证。 |
| `OTHER` / `UNKNOWN` | 已区分：低分且两类均无几何 inlier 返回 `OTHER`；模糊或有部分证据但不足以确定时返回 `UNKNOWN`。仍需真实负例冻结集校准边界。 |
| 公开 GitHub | `AceYKN/nailoong-naiwa` 已确认公开；当前提交 `9110402ea511aa05a6351bb0afcc52f0b28e1605` 的 GitHub CI run `34872691473` 中，仓库卫生、前端、Rust Ubuntu、Rust Windows、Windows Tauri 打包和 Windows OpenCV 原生 feature job 全部成功。 |
