...
| Rust 决策层 | `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml --all-targets`：61 项库测试通过；覆盖 pHash、参考图数量、分数、普通分类几何证据门槛、`OTHER/UNKNOWN`、置信度、动画严格门槛、WebP 静态/动画回退、存储、QQ 服务状态和严格门槛。新增回归测试确认普通构建拒绝 `AUTO_RECALL`，并将旧数据库中的该模式降级为 `OBSERVE`。 |
| Rust 质量门 | 默认 feature 的 fmt/clippy 通过；`opencv-backend,auto-recall-release` 组合的 74 项库测试与 clippy 也通过。 |
