...
- [x] 公共 CI 提供可复现且固定版本的 Windows OpenCV/Clang 原生 feature job（OpenCV 4.13.0 + LLVM 20.1.8；最近一次代码验证提交 `c1b04a376051bb7ce78c42eb80e209ad2f352c48` 的 CI run `34886669194` 已通过 check/test/clippy、Tauri OpenCV NSIS、runtime staging 与资源检查；随后文档同步提交 `cfc83c2286204fdec47c01e522458e547cff67cb` 的 CI run `34888421124` 也已全部通过）；默认便携 job 仍不依赖机器专属原生工具链
- [x] 成功的 OpenCV CI run 上传短期 Windows 验证 artifact（最新公开 HEAD `cfc83c2286204fdec47c01e522458e547cff67cb` 的 run `34888421124` 上传 `nlnf-windows-opencv-cfc83c2286204fdec47c01e522458e547cff67cb`，46,879,078 bytes，保留至 2026-09-28；包含 NSIS 安装包、exe 和匹配 runtime DLL；不等同于正式 Auto Recall release）
